...
class gravityfield:
    ...
    def __init__(self, bodies, angular_vel, lim=3/2,  n=500):
        ...
        self.xlim = self.ylim = lim * \
            max([abs(body["position"]) for body in self.bodies])
        self.n = n

        self.x = np.linspace(-self.xlim, self.xlim, self.n)
        self.y = np.linspace(-self.ylim, self.ylim, self.n)
        self.z = np.array([[x + 1j*y for y in self.y] for x in self.x])

        self.g = self.acceleration_relative(
            self.bodies, self.angular_vel, self.z)
    ...
    def acceleration_gravity(self, body, position):
        return (
            ...
            (np.abs(position - body["position"]) >= body["radius"]) *
            (- body["mu"] / (np.conj(position - body["position"])
                             * np.abs(position - body["position"])))
        )
...
def main():
    ...
    earth_moon.contour(title=contour_title, masking=1.5)
